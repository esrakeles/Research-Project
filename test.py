...
#from mysql_connect import database_connector
...
mycursor.execute("SELECT * FROM sys.licence where Licence")
...
myNetwork = OCR(modelFile="model/model.h5",
...
def recognized_plate(list_char_on_plate, size):
    ...
    for x in myresult:
       if (x[1] == final_plate):
           print("eslesen plaka degeri ", x[1])
           print("eslesti")
           status = str(x[2])
           print("status->" + status)
    ...
    with open("Output.txt", "a") as text_file:
        print("{}".format(final_plate), file=text_file)
...
if __name__=="__main__":
    while(cap.isOpened()):
        ...
        cv2.rectangle(_frame, (190, 0), (380, 40), (0, 0, 0), -1)
        if(licence_status == "SUSPECT"):
            cv2.putText(_frame, licence_status, (220, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
        else:
            cv2.putText(_frame, licence_status, (220, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        # out.write(_frame)
        ...
        if possible_plates is not None:
            ...
            if (distance < 100):
                if(countPlates < countPlates_threshold):
                    ...
                    threading.Thread(target=recognized_plate, args=(list_char_on_plate, 80)).start()
                    countPlates += 1
            else:
                countPlates = 0
                list_char_on_plate = []
